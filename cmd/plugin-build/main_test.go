...
func TestCopyHookAssets(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(root, "script", "ready.sh"), body, 0755); err != nil {
		t.Fatal(err)
	}
	if err := copyHookAssets(root, out, pluginHooks{Install: "script/install.sh", Uninstall: "script/uninstall.sh", Ready: "script/ready.sh"}); err != nil {
		t.Fatal(err)
	}
	...
	ready := filepath.Join(out, "script-ready.sh")
	got, err = os.ReadFile(ready)
	if err != nil {
		t.Fatal(err)
	}
	if string(got) != string(body) {
		t.Fatalf("ready copied=%q", got)
	}
	...
	if hookAssetName("script/ready.sh") != "script-ready.sh" {
		t.Fatal(hookAssetName("script/ready.sh"))
	}
...
func TestRepoFlynnPluginManifestUninstallHook(t *testing.T) {
	...
	if m.Hooks.Ready != "script/ready.sh" {
		t.Fatalf("hooks.ready=%q", m.Hooks.Ready)
	}
	if _, err := os.Stat(filepath.Join(root, "script", "uninstall.sh")); err != nil {
		t.Fatal(err)
	}
	if _, err := os.Stat(filepath.Join(root, "script", "ready.sh")); err != nil {
		t.Fatal(err)
	}
	if hookAssetName(m.Hooks.Uninstall) != "script-uninstall.sh" {
		t.Fatalf("asset name=%q", hookAssetName(m.Hooks.Uninstall))
	}
	if hookAssetName(m.Hooks.Ready) != "script-ready.sh" {
		t.Fatalf("ready asset name=%q", hookAssetName(m.Hooks.Ready))
	}
}
...
func TestRepoHasNoRuntimeDockerCompose(t *testing.T) {
	...
	for _, name := range []string{"Dockerfile", "docker-compose.yml"} {
		if _, err := os.Stat(filepath.Join(root, name)); err == nil {
			t.Fatalf("runtime %s must not exist; this is a Flynn plugin", name)
		}
	}
}
